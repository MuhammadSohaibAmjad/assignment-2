...
int main(){
	...
	int grid[20][20] = { {8,02,22,97,38,15,00,40,00,75,04,05,07,78,52,12,50,77,91,8},
                         ...
                         {20,73,35,29,78,31,90,01,54,31,49,71,48,86,81,16,23,57,05,54},
...
//horizontal
   ...
   col_index=j;
    ...
    for (int i=0;i<=20-4;++i) {
        for (int j=0;j<=20-4;++j) {
            ...
            product=1;
            for (int k=0;k<4;++k) {
                product=product*grid[i + 3 - k][j + k];
            }
            if (product>maxproduct) {
            maxproduct=product;
            row_index=i;
            ...
            a=1;}}}
    if(a==1){
    	printf("bottom left to top right\n");
	}
	else if(a==2){
   ...
   printf("Maximum product of four adjacent numbers: %lld\n",maxproduct);
   return 0;
}
